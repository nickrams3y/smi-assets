...
function pricePromo(){
 ...
 set(box,{display:"grid","grid-template-columns":"max-content minmax(0, 1fr)","grid-template-areas":'"sale-label sale-label" "price comparison" "promotion promotion" "paypal paypal"',"align-items":"center","column-gap":"12px","row-gap":"7px"});set(box.querySelector(".product-details__product-price"),{"grid-area":"price"});set(sale,{display:"contents"});set(label,{"grid-area":"sale-label","justify-self":"start"});set(compare,{"grid-area":"comparison",margin:"0"});set(paypal,{"grid-area":"paypal",width:"100%"});
...
function home(){
 ...
 f.addEventListener("submit",function(e){e.preventDefault();var k=m.value,a=base(),v=+y.value,z=s.value;if(!b.value||!k||!a||!v||!z){show("Please complete all four fields","Select a make, model, year and current system.",0,1);return}
  if(z==="4"){
   if(b.value==="Ford"&&k==="F-150"&&v>=2018&&v<=2020){show("We have a complete kit for your F-150","This kit includes all of the hardware and programming needed to upgrade your factory 4&quot; system to Sync 3.",[["View the F-150 Kit",U.f4],["12&quot; Upgrade Kit",U.t]]);return}
   if(b.value==="Ford"&&(k==="F-250"||k==="F-350")&&v>=2020&&v<=2022){show("We have a complete kit for your Super Duty","This kit includes all of the hardware and programming needed to upgrade your factory 4&quot; system to Sync 3.",[["View the Super Duty Kit",U.sd4],["12&quot; Upgrade Kit",U.t]]);return}
...
function init(){
 ...
 function years(){var k=key(),a=k&&V[b.value][k];y.innerHTML='<option value="">Select model year</option>';y.disabled=!a;if(a)for(var i=a[0];i<=a[1];i++){var o=document.createElement("option");o.value=o.textContent=i;y.appendChild(o)}system()}
 ...
 if(isApim){
  ...
  if(z==="4"){show("You need a complete conversion kit","A replacement APIM alone will not upgrade a factory 4&quot; base system. Use our conversion guide to identify the additional hardware and programming your vehicle requires.",[["Find a Complete Upgrade",P+"sync-2-to-sync-3-upgrades"],["Read the 4&quot; to 8&quot; Guide",U.g]],1);return}
