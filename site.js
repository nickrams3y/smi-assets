...
function pricePromo(){
 ...
 if(isTwelve){if(existing)existing.remove();if(box.dataset.smPricePromoLayout==="1"){var clear=function(el,props){if(!el)return;props.forEach(function(k){el.style.removeProperty(k)})};clear(box,["display","grid-template-columns","grid-template-areas","align-items","column-gap","row-gap"]);clear(box.querySelector(".product-details__product-price"),["grid-area"]);clear(box.querySelector(".product-details__product-on-sale"),["display"]);clear(box.querySelector(".product-details__label-container"),["grid-area","justify-self"]);clear(box.querySelector(".details-product-price-compare__container"),["grid-area","margin"]);clear(box.querySelector(".ec-paypal-pay-later-message"),["grid-area","width"]);delete box.dataset.smPricePromoLayout}return}
 ...
 set(box,{display:"grid","grid-template-columns":"max-content minmax(0, 1fr)","grid-template-areas":'"sale-label sale-label" "price comparison" "promotion promotion" "paypal paypal"',"align-items":"center","column-gap":"12px","row-gap":"7px"});box.dataset.smPricePromoLayout="1";set(box.querySelector(".product-details__product-price"),{"grid-area":"price"});set(sale,{display:"contents"});set(label,{"grid-area":"sale-label","justify-self":"start"});set(compare,{"grid-area":"comparison",margin:"0"});set(paypal,{"grid-area":"paypal",width:"100%"});
